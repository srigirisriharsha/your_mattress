...
const AdminBulkOrders = () => {
  ...
  const _getAllBulkOrders = useCallback(async () => {
    try {
      if (employeesLoaded && employeesData?.length > 0) {
        let response;
        if (adminUserData?.role === 'Admin') {
          response = await getAllBulkOrders();
        } else {
          response = await getEmployeeAllBulkorders(admin);
        }
  ...
  useEffect(() => {
    if (employeesLoaded) {
      _getAllBulkOrders(); // Only fetch contact data when employees are fully loaded
    }
  }, [employeesLoaded, _getAllBulkOrders]);
  ...
  const handleSearch = (searchTerm) => {
    ...
    const filteredData = contactusData.filter((user) => {
      ...
      const user_mobile = user.mobile?.toString().toLowerCase() || '';
  ...
  const goToProductView = async (user) => {
    navigate(`/admin/bulk-orders/${user?.tableDataObj?.id}`, {
                  ...
                  {currentItems?.length > 0 ? (
                    currentItems?.map((e, i) => (
                      <tr key={i} className="bg-white border-b">
                        ...
                        >
                          <div className="relative flex inline-block gap-2">
                            ...
                            <p
                              ...
                              title={e?.tableDataObj?.name} // Tooltip to show full text on hover
                            >
                              {e?.tableDataObj?.name?.length > 15
                                ? `${e?.tableDataObj?.name.slice(0, 15)}...`
                                : e?.tableDataObj?.name}
                            </p>
                          </div>
                        </td>
                        <td
                          className="px-6 py-4 w-[300px] whitespace-nowrap  "
                          title={e?.tableDataObj?.message}
                        >
                          {limitedText(e?.tableDataObj?.message)}
                        </td>
                        <td
                          className="px-6 py-4 w-[300px] whitespace-nowrap  "
                          title={e?.tableDataObj?.email}
                        >
                          {limitedText(e?.tableDataObj?.email)}
                        </td>
                        <td className="px-6 py-4 w-[300px] whitespace-nowrap  ">
                          {e?.tableDataObj?.mobile}
                        </td>
                        <td className="px-6 py-4 w-[300px] whitespace-nowrap ">
